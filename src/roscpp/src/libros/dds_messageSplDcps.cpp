...
/* Code generated by /root/OpenSpliceDDS6.4/src/tools/idlpp/code/idl_genCorbaCxxCopyin.c at line 2411 */
    ...
    {
    /* Code generated by /root/OpenSpliceDDS6.4/src/tools/idlpp/code/idl_genCorbaCxxCopyin.c at line 1873 */
            c_octet *buf0;
    ...
    struct _ROSDDS_Msg *to)
{
    ...
    to->version = (c_ulong)from->version;
#ifdef OSPL_BOUNDS_CHECK
    if(from->callerId){
        to->callerId = c_stringNew(base, from->callerId);
    } else {
        OS_REPORT (OS_ERROR, "copyIn", 0,"Member 'ROSDDS::Msg.callerId' of type 'c_string' is NULL.");
        result = OS_C_FALSE;
    }
#else
    to->callerId = c_stringNew(base, from->callerId);
#endif
    {
        typedef c_octet _DestType[8];
        _DestType *dest = &to->reserved;
        memcpy (dest, from->reserved, sizeof (*dest));
    }
    if(result){
    ...
    void *_to)
{
    ...
    to->version = (::DDS::ULong)from->version;
    to->callerId = DDS::string_dup(from->callerId ? from->callerId : "");
    {
        typedef c_octet _DestType[8];
        _DestType *src = &from->reserved;

        memcpy (to->reserved, src, sizeof (*src));
    }
    {
